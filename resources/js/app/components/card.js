...
export const card = (() => {
    ...
    const renderHeader = (c) => {
        ...
        const hiddenItem = showHide
            .get("hidden")
            .find((i) => i.uuid === c.uuid);

        const isHidden = hiddenItem ? !hiddenItem.show : true;

        return `class="${isHidden ? "d-none" : ""} overflow-x-auto mw-100 border-start bg-theme-auto py-2 ps-2 pe-0 my-2 ms-2 me-0"`;
    };
    ...
    <hr class="my-1">`;
        ...
        <div class="d-flex justify-content-center align-items-center my-2">
            <img
                src="${await gif.get(c.gif_url)}"
                id="img-gif-${c.uuid}"
                class="img-fluid mx-auto gif-image rounded-4"
                alt="selected-gif">
        </div>`
            );
        }
        ...
        const data = util.convertMarkdownToHTML(
            util.escapeHtml(
                moreMaxLength
    ...
    <p
        ...
        style="white-space: pre-wrap !important; font-size: 0.95rem;"
        data-comment="${util.base64Encode(commentText)}"
        id="content-${c.uuid}">
    ...
    ${
        moreMaxLength
            ? `
            <p class="d-block mb-2 mt-0 mx-0 p-0">
                <a
                    class="text-theme-auto"
                    role="button"
                    style="font-size: 0.85rem;"
                    data-show="false"
                    onclick="undangan.comment.showMore(this, '${c.uuid}')">

...
export const comment = (() => {
            ...
            .then((res) => {
                ...
                if (res.status === "success") {
                    if (res.City.length !== 0 && res.RegionName.length !== 0) {
                        ...
                        res.Capital.length !== 0 &&
                        res.CountryName.length !== 0
                    ) {
        ...
        return request(
            ...
            .then(async (res) => {
                ...
                if (res.data.lists && session.isAdmin()) {
                    await Promise.all(
                        res.data.lists.map((v) => fetchTracker(v)),
                    );
                }
    ...
    const update = async (button) => {
        ...
        const gifUuid = id ? id : gif.default;

        const gifIsOpen = gif.isOpen(gifUuid);

        const gifId = gif.getResultId(gifUuid);
        ...
        const status = await request(
            HTTP_PUT,
            `/api/v2/comment/${owns.get(id)}?lang=${lang.getLanguage()}`,
        )
            .token(session.getToken())
            .body({
                ucapan: form.value.trim(),
                keterangan: isPresent ? "hadir" : "tidak_hadir",
            })
            .send(dto.statusResponse)
            .then((res) => res.data.status);
    ...
    const send = async (button) => {
        ...
        if (!form || form.value.trim().length === 0) {
            util.notify("Message cannot be empty.").warning();
            return;
        }
        ...
        if (!session.isAdmin()) {
            ...
            info.set("phone", phoneValue);
            info.set("address", addressValue);
        }

        const response = await request(
            HTTP_POST,
            `/api/v2/comment/?lang=${lang.getLanguage()}`,
        )
            .token(session.getToken())
            .body({
                nama_tamu: nameValue,

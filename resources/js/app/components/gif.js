...
export const gif = (() => {
 ...
 const render = (uuid, path, params) => {
     params = {
         ...
         api_key:
             config.get("tenor_key") ||
             "4bWn3PJNBOTPPGWpS3uzqJNusq0UYwq7LqTbkFdrZb0tXOZBZ6Hy22GJZCFCR1rk",
     ...
     ctx.last = request(HTTP_GET, `https://api.klipy.com/v2${path}?${param}`)
         .withCache()
         .withRetry()
    ...
    const click = async (button, uuid, id, urlBase64) => {
        ...
        // SIMPAN URL GIF
        res.setAttribute("data-url", util.base64Decode(urlBase64));

        // OPTIONAL: tetap simpan id
        res.setAttribute("data-id", id);

        res.querySelector(`#gif-cancel-${uuid}`).classList.replace(
            "d-none",
            "d-flex",
        );
